...
void usage(void)
{
	cout << endl<<" Usage: simulateCFHTobs [...options...]" << endl<<endl;

	cout << " -i : INFILE: input filename of catalog generated by baseSimulation "<<endl;
	cout << " -o : OUTFILE: output filename for observed CFHT catalog (will be stored in output/)"<<endl;
	...
	cout << " -e : FLUXERR: percentage error on observed flux [DEFAULT=0.03]"<<endl;
...
{
	cout << " ==== simulateCFHTobs.cc program , to simulate LSST data ==== "<<endl;
	...
	string outfile="output/simulateCFHTobs.fits";
	...
	char c;
	while((c = getopt(narg,arg,"hrmi:o:e:n:t:")) != -1)  {
	    switch (c) {
            case 'i' :
		        infile = optarg;
		        break;
    ...
    cout << "     Percentage flux error = "<< fluxErr <<endl;
    cout << endl;
	...
	cout <<"     Load in CFHT filters"<<endl;
	...
	vector<Filter*> CFHTfilters=readFilterList.getFilterArray();
	...
	SimData simgal(sedArray,CFHTfilters,su,rg,nElliptical,nSpiral);
    cout << endl;
	...
	for (int i=0; i<ng; i++) {
        ...
        // Calculate galaxy magnitude in observed filters ugriz
		...
		vector<double> uObservation = simgal.addError(uMagTh,fluxErr,0);
		vector<double> gObservation = simgal.addError(gMagTh,fluxErr,1);
		vector<double> rObservation = simgal.addError(rMagTh,fluxErr,2);
		vector<double> iObservation = simgal.addError(iMagTh,fluxErr,3);
		vector<double> zObservation = simgal.addError(zMagTh,fluxErr,4);
		...
		rowin[10]=gObservation[0];
		rowin[11]=rObservation[0];
		rowin[12]=iObservation[0];
		rowin[13]=zObservation[0];
    ...
    cerr << " simulateCFHTobs.cc: Catched Exception (PThrowable)" 
	 << (string)typeid(exc).name() 
         << "\n...exc.Msg= " << exc.Msg() << endl;
    rc = 99;
  }
  catch (std::exception & e) {  // catching standard C++ exceptions
    cerr << " simulateCFHTobs.cc: Catched std::exception "  << " - what()= " 
	 << e.what() << endl;
    rc = 98;
  }
  catch (...) {  // catching other exceptions
    cerr << " simulateCFHTobs.cc: some other exception (...) was caught ! " 
	 << endl;
    rc = 97;
  }
  cout << " ==== End of simulateCFHTobs.cc program  Rc= " << rc << endl;
  return rc;	
}
